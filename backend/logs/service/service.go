...
import (
	...
	"quickpulse/backend/logs/collector"
	"quickpulse/backend/logs/parser"
	"quickpulse/backend/logs/pubsub"
	"quickpulse/backend/logs/store"
)
...
func Start(ctx context.Context, db *sql.DB, hostName string) (*Service, error) {
	...
	if err != nil {
		log.Printf("[logs] settings load failed, using defaults: %v", err)
	...
	if d, err := collector.NewDocker(reg, joiner, submitter, hostName, env); err != nil {
		log.Printf("[logs/docker] disabled: %v", err)
	} else {
		go d.Run(ctx)
		log.Printf("[logs/docker] started")
	}

	// K8s collector: same — kubeconfig may be absent.
	k := collector.NewK8s(reg, joiner, submitter, env)
	go k.Run(ctx)
	log.Printf("[logs/k8s] started")
...
func (s *Service) runJanitor(ctx context.Context) {
	...
	runOnce := func() {
		...
		if err != nil {
			log.Printf("[logs/janitor] vacuum: %v", err)
			return
		}
		if n > 0 {
			log.Printf("[logs/janitor] evicted %d rows", n)
		}
	}
	...
	for {
		...
		case <-checkpointTick.C:
			...
			if _, err := walCheckpoint(cctx); err != nil {
				log.Printf("[logs/janitor] wal_checkpoint: %v", err)
			}
			cancel()
		}
	}
