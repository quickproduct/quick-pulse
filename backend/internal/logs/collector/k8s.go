...
func (c *K8sCollector) Run(ctx context.Context) {
	...
	if cfg, err := rest.InClusterConfig(); err == nil {
		if cs, err := kubernetes.NewForConfig(cfg); err == nil {
			zap.L().Sugar().Infof("[logs/k8s] watching in-cluster context")
	...
	path := kubeconfigPath()
	if path == "" {
		zap.L().Sugar().Infof("[logs/k8s] no kubeconfig found, k8s log collection disabled")
		return
	}
	apiCfg, err := clientcmd.LoadFromFile(path)
	if err != nil {
		zap.L().Sugar().Infof("[logs/k8s] failed to load kubeconfig %s: %v", path, err)
		return
	}
	if len(apiCfg.Contexts) == 0 {
		zap.L().Sugar().Infof("[logs/k8s] kubeconfig %s has no contexts", path)
		return
	}
	...
	for ctxName := range apiCfg.Contexts {
		...
		if err != nil {
			zap.L().Sugar().Infof("[logs/k8s] context %q: clientset build failed: %v", ctxName, err)
			continue
		}
		zap.L().Sugar().Infof("[logs/k8s] watching context %q", ctxName)
		go c.watchClusterLoop(ctx, ctxName, cs)
		started++
	}
	if started == 0 {
		zap.L().Sugar().Infof("[logs/k8s] no contexts could be reached")
	}
}
...
func (c *K8sCollector) watchClusterLoop(ctx context.Context, cluster string, cs *kubernetes.Clientset) {
	...
	for {
		...
		if err := c.watchOnce(ctx, cluster, cs); err != nil && !errors.Is(err, context.Canceled) {
			zap.L().Sugar().Infof("[logs/k8s] %s: watch error: %v — backing off %s", cluster, err, backoff)
...
func (c *K8sCollector) stream(ctx context.Context, cs *kubernetes.Clientset, namespace, pod, container string, meta parser.SourceMeta) {
	...
	for {
		...
		if err != nil {
			...
			zap.L().Sugar().Infof("[logs/k8s] %s/%s: open failed: %v", namespace, pod, err)
			if !sleepCtx(ctx, backoff) {
				return
			}
