...
func (c *DockerCollector) discoverOnce(ctx context.Context) {
	...
	if err != nil {
		zap.L().Sugar().Infof("[logs/docker] discover: list failed: %v", err)
		return
	}
		...
		for {
			...
			case err := <-errChan:
				if err != nil && !errors.Is(err, context.Canceled) {
					zap.L().Sugar().Infof("[logs/docker] event stream error: %v — reconnecting", err)
				}
				break loop
			}
		}
...
func (c *DockerCollector) stream(ctx context.Context, id string, meta parser.SourceMeta) {
	...
	for {
		...
		if err != nil {
			zap.L().Sugar().Infof("[logs/docker] %s: open failed: %v", meta.Container, err)
