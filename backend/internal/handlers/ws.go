...
func HandleWSChannel(channel string) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			zap.L().Sugar().Infof("WS upgrade failed for channel %s: %v", channel, err)
			return
		}

		zap.L().Sugar().Infof("WS client connected to channel %s: user %s", channel, userID)
...
func HandleWSLogs(w http.ResponseWriter, r *http.Request) {
	...
	conn, err := upgrader.Upgrade(w, r, nil)
	if err != nil {
		zap.L().Sugar().Infof("WS upgrade failed for container logs: %v", err)
		return
	}
...
func HandleWSContainerTerminal(w http.ResponseWriter, r *http.Request) {
	...
	conn, err := upgrader.Upgrade(w, r, nil)
	if err != nil {
		zap.L().Sugar().Infof("Terminal WS upgrade failed: %v", err)
		return
	}
	defer conn.Close()
