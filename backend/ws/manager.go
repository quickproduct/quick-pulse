...
func (m *WebSocketManager) Connect(channel string, conn *websocket.Conn) {
	...
	log.Printf("WS Connected to channel %s. Total: %d", channel, len(m.connections[channel]))
}
...
func (m *WebSocketManager) Disconnect(channel string, conn *websocket.Conn) {
	...
	conn.Close()
	log.Printf("WS Disconnected from channel %s", channel)
}
...
func (m *WebSocketManager) Broadcast(channel string, data interface{}) {
	...
	for _, conn := range conns {
		...
		if err != nil {
			log.Printf("WS write failed to channel %s, removing connection: %v", channel, err)
