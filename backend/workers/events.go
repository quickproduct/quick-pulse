...
func StartEventsWorker() {
	log.Printf("Starting Docker events worker")
	go func() {
		consecutiveErrors := 0
		for {
			...
			if err != nil {
				consecutiveErrors++
				log.Printf("Events worker error (consecutive %d): %v", consecutiveErrors, err)
...
func listenToDockerEvents() error {
	...
	for {
		...
		case msg := <-msgChan:
			...
			if err != nil {
				log.Printf("Warning: failed to store container event: %v", err)
			}
