<script lang="ts">
	...
	import { getPodLogs } from '$lib/api/kubernetes';
	import { getAccessToken } from '$lib/api/client';

	let namespace: string = $derived(page.params.namespace || '');
	let name: string = $derived(page.params.name || '');
	let context: string = $derived(page.url.searchParams.get('context') ?? '');
	...
	async function loadInitialLogs() {
		loading = true;
		try {
			const result = await getPodLogs(namespace, name, tail, context || undefined);
			logs = result?.logs || [];
		} catch (e: any) {
	...
	function connectWS() {
		...
		const ctxParam = context ? `&context=${encodeURIComponent(context)}` : '';
		ws = new WebSocket(`${protocol}//${host}/ws/kubernetes/logs/${namespace}/${name}?token=${token}${ctxParam}`);
		...
		ws.onmessage = (event) => {
			...
			try {
				...
				console.warn('[ws/k8s-logs] parse error:', e);
			}
		};

		ws.onerror = (event) => {
			console.error('[ws/k8s-logs] error:', event);
			wsConnected = false;
		};
...
<svelte:head>
	<title>Logs - {namespace}/{name} - QuickPulse</title>
</svelte:head>

<div class="flex items-center justify-between mb-4 flex-wrap gap-3 max-w-7xl mx-auto">
	<div>
		<a href="/kubernetes" class="text-xs text-[var(--qp-accent)] hover:underline">&larr; Back to Kubernetes</a>
		<h1 class="text-xl font-semibold text-white mt-1">Live Pod Logs</h1>
		<p class="text-sm text-[var(--qp-text-muted)] font-mono">{namespace} / {name}</p>
	</div>
